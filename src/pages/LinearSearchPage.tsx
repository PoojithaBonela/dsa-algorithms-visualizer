...
import { generateLinearSearchTrace } from '../lib/algorithms/searching';
import type { AlgorithmTrace } from '../lib/algorithms/types';

interface LinearSearchPageProps {
    onBack: () => void;
    onHome: () => void;
    onCatalog: () => void;
}

export const LinearSearchPage: React.FC<LinearSearchPageProps> = ({ onBack, onHome, onCatalog }) => {
    ...
    const handleVisualize = () => {
        try {
            ...
            setTrace(generateLinearSearchTrace(numbers, targetNum));
            setError(null);
        } catch (err: any) {
    ...
    const handleRandomize = () => {
        ...
        const randomTarget = randomNumbers[Math.floor(Math.random() * randomNumbers.length)]; // Ensure target exists sometimes

        setInput(randomNumbers.join(', '));
        setTarget(Math.random() > 0.3 ? randomTarget.toString() : (Math.floor(Math.random() * 90) + 10).toString()); // 70% chance to exist
        setTrace(null);
        setError(null);
    };
            ...
            <header className="h-20 border-b border-white/10 backdrop-blur-md bg-[#050505]/80 flex items-center justify-between px-6 sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h1 className="font-black text-xl tracking-tight text-white uppercase leading-none">Linear Search <span className="text-primary text-xs ml-2">Active Insight</span></h1>
                    </div>
                </div>
            ...
            <main className="flex-1 flex flex-col p-6 gap-6 relative z-10 max-w-6xl mx-auto w-full">
                ...
                {!trace ? (
                    ...
                    >
                        <div className="w-full max-w-2xl p-10 bg-[#1a1a1a] rounded-[2.5rem] border border-white/10 ring-1 ring-white/5 shadow-2xl space-y-8">
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-white/40 text-sm">Enter the array and the target number you want to find.</p>
                            </div>
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="px-3 py-1 bg-primary/10 border border-primary/20 rounded-full">
                                                <span className="text-[10px] font-black text-primary">T: O(n)</span>
                                            </div>
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                        ...
                        <SearchVisualizer
                            trace={trace}
                            onReset={() => setTrace(generateLinearSearchTrace(input.split(',').map(n => parseInt(n.trim())), parseInt(target)))}
                        />
                    </div>
                )}
            </main>
