...
import { generateBinarySearchTrace } from '../lib/algorithms/searching';
import type { AlgorithmTrace } from '../lib/algorithms/types';

interface BinarySearchPageProps {
    onBack: () => void;
    onHome: () => void;
    onCatalog: () => void;
}

export const BinarySearchPage: React.FC<BinarySearchPageProps> = ({ onBack, onHome, onCatalog }) => {
    const [input, setInput] = useState<string>('64, 34, 25, 12, 22, 11, 90');
    const [target, setTarget] = useState<string>('22');
        ...
        try {
            const numbers = input
                ...
                .map(n => {
                    ...
                    if (isNaN(parsed)) throw new Error(`Invalid number in array: ${n}`);
                    return parsed;
                });
            ...
            setTrace(generateBinarySearchTrace(numbers, targetNum));
            setError(null);
        } catch (err: any) {
    ...
    const handleRandomize = () => {
        ...
        // For binary search input doesn't STRICTLY need to be sorted by user (we sort it), but random generation
        // just gives random numbers.
        const randomTarget = randomNumbers[Math.floor(Math.random() * randomNumbers.length)];

        setInput(randomNumbers.join(', '));
        setTarget(Math.random() > 0.3 ? randomTarget.toString() : (Math.floor(Math.random() * 90) + 10).toString());
        setTrace(null);
        setError(null);
    };
        ...
        <div className="min-h-screen bg-[#050505] text-white flex flex-col font-sans selection:bg-primary/30 relative">
            ...
            <header className="h-20 border-b border-white/10 backdrop-blur-md bg-[#050505]/80 flex items-center justify-between px-6 sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <span className="text-[10px] font-black uppercase tracking-widest text-white/40 block leading-none mb-1">Searching Algorithms</span>
                        <h1 className="font-black text-xl tracking-tight text-white uppercase leading-none">Binary Search <span className="text-primary text-xs ml-2">Active Insight</span></h1>
                    </div>
                </div>
            ...
            <main className="flex-1 flex flex-col p-6 gap-6 relative z-10 max-w-6xl mx-auto w-full">
                ...
                {!trace ? (
                    ...
                    >
                        <div className="w-full max-w-2xl p-10 bg-[#1a1a1a] rounded-[2.5rem] border border-white/10 ring-1 ring-white/5 shadow-2xl space-y-8">
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-white/40 text-sm">Enter the array and the target number. <span className="text-primary">Note: Array will be automatically sorted.</span></p>
                            </div>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <label className="text-[10px] uppercase font-black tracking-widest text-white/40 ml-2">Target Number</label>
                                    <div className="relative group">
                                        <input
                                            type="text"
                                            value={target}
                                    ...
                                    <div>
                                        ...
                                        <div className="flex items-center gap-4">
                                            <div className="px-3 py-1 bg-primary/10 border border-primary/20 rounded-full">
                                                <span className="text-[10px] font-black text-primary">T: O(log n)</span>
                                            </div>
                                            <div className="px-3 py-1 bg-emerald-500/10 border border-emerald-500/20 rounded-full">
                                                <span className="text-[10px] font-black text-emerald-400">S: O(1)</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full inline-block">
                                            <span className="text-[10px] font-black text-amber-400">T: O(1)</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                            ...
                            <div className="flex items-center gap-6">
                                <div className="space-y-1">
                                    ...
                                    <div className="flex gap-2">
                                        {/* Show sorted Input if trace exists, else input string */}
                                        {trace.steps[0].array.slice(0, 8).map((n, i) => (
                                            <span key={i} className="text-xs font-mono text-white/60">{n}</span>
                                        ))}
                                        {/* Target Indicator */}
                                        <span className="text-xs font-mono text-primary ml-2 border-l border-white/10 pl-2">Target: {target}</span>
                        ...
                        <SearchVisualizer
                            trace={trace}
                            onReset={() => setTrace(generateBinarySearchTrace(input.split(',').map(n => parseInt(n.trim())), parseInt(target)))}
                        />
                    </div>
                )}
            </main>
