...
import { generateSelectionSortTrace } from '../lib/algorithms/sorting';
import type { AlgorithmTrace } from '../lib/algorithms/types';

interface SelectionSortPageProps {
    onBack: () => void;
    onHome: () => void;
    onCatalog: () => void;
}

export const SelectionSortPage: React.FC<SelectionSortPageProps> = ({ onBack, onHome, onCatalog }) => {
    const [input, setInput] = useState<string>('64, 25, 12, 22, 11');
    ...
    const handleVisualize = () => {
        try {
            ...
            setTrace(generateSelectionSortTrace(numbers));
            setError(null);
        } catch (err: any) {
    ...
    const handleRandomize = () => {
        ...
        setTrace(generateSelectionSortTrace(randomNumbers));
        setError(null);
    };
            ...
            <header className="h-20 border-b border-white/10 backdrop-blur-md bg-[#050505]/80 flex items-center justify-between px-6 sticky top-0 z-50">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <h1 className="font-black text-xl tracking-tight text-white uppercase leading-none">Selection Sort <span className="text-primary text-xs ml-2">Active Insight</span></h1>
                    </div>
                </div>
            ...
            <main className="flex-1 flex flex-col p-6 gap-6 relative z-10 max-w-6xl mx-auto w-full">
                ...
                {!trace ? (
                                ...
                                <div className="grid grid-cols-2 gap-8">
                                    ...
                                    <div>
                                        ...
                                        <div className="px-3 py-1 bg-amber-500/10 border border-amber-500/20 rounded-full inline-block">
                                            <span className="text-[10px] font-black text-amber-400">T: O(n²)</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className="flex-1 flex flex-col gap-6">
                        ...
                        <Visualizer
                            trace={trace}
                            onReset={() => setTrace(generateSelectionSortTrace(input.split(',').map(n => parseInt(n.trim()))))}
                        />
                    </div>
                )}
            </main>
